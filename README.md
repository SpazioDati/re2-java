...
Anyway this patch works OK in the majority of "normal" cases of usage of `\b` near word characters. For example, without using the option, the regex `\bball\b` would match "ball" in the text "Fußball" (because "ß" is a non-word in ascii). With the option `UNICODE_WORD` we can transparently avoid this match.
